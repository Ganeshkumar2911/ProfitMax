...
var MatrixTicker = function (params) {
	...
	this.connect = function () {
		...
		if (config.env == "production") {
			url = 'wss://bullback.xtention.in/api/v1/live/ws' + '?token=' + params.token
		...
		ws.onopen = function () {
			...
			read_timer = setInterval(function () {
				try {
					// send() will stringify objects and check ws state
					send({ type: 'ping' })
					last_read = new Date()
				} catch (e) {
					// keep the same behaviour — closing will be handled elsewhere
				}
